...
function App() {
    ...
    useEffect(() => {
        userService.getAll().then(initialUsersList => {
            ...
            setUsers([]);
            console.error('Error during getting the data about users from the server');
        }).catch((err) => {
            setUsers([]);
            console.error(err);
        });

        eventService.getAll().then(initialEventsList => {
            // Log to see what cames from backend
            ...
            setEventsList([]);
            console.error('Error during getting the data about events from the server');
        }).catch((err) => {
            setEventsList([]);
            console.error(err);
        });
                  ...
                  (
                      <main className="main-container">
                          <div className="interface-box">
                              <UserInterface users={users} setUsers={setUsers} events={eventsList} setEvents={setEventsList}/>
                          </div>
